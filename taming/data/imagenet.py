...
from taming.data.base import ImagePaths, ImagePathsTorchVision
...
class ImageNetBase(Dataset):
    def __init__(self, config=None, root_path=None, folder_name=None, argen_root=None):
        self.config = config or OmegaConf.create()
        if not type(self.config) == dict:
            self.config = OmegaConf.to_container(self.config)
        self.root_path = root_path
        self.folder_name = folder_name
        self.argen_root = argen_root
        self._prepare()
        self._prepare_synset_to_human()
        self._prepare_idx_to_synset()
        self._prepare_relpath_to_argenidx()
        self._load()
    ...
    def _prepare_relpath_to_argenidx(self):
        self.argen_relpath_file = os.path.join(self.argen_root, "all_paths.txt")
    ...
    def _load(self):
        ...
        with open(self.argen_relpath_file, "r") as f:
            self.argen_relpaths = f.read().splitlines()
        self.relpath_to_argenidx = {_path: idx for idx, _path in enumerate(self.argen_relpaths)}
        argenidxs = []
        for _relpath in self.relpaths:
            argenidxs.append(self.relpath_to_argenidx[_relpath])
        ...
        labels = {
            ...
            "argen_tokens": self.argen_tokens,
            "arin_tokens": self.arin_tokens,
        }
        self.data = ImagePathsTorchVision(self.abspaths,
                                          labels=labels,
...
class ImageNetTrain(ImageNetBase):
    ...
    def _prepare(self):
        ...
        print(self.root_path, self.folder_name)
